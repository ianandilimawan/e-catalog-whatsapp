...
class StoreTable extends PowerGridComponent
{
    use WithExport;
    public string $tableName = 'store-table';
    ...
    public function setUp(): array
    {
        ...
        return [
            PowerGrid::exportable('export_stores_' . now()->format('Ymd_His'))
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('link', function (Store $row) {
                $url = route('catalog.show', $row->slug);
                return '<a href="' . $url . '" target="_blank" class="text-blue-600 hover:text-blue-800 hover:underline font-medium inline-flex items-center gap-1">
                            Buka Toko
            ...
            ->add('logo')
            ->add('banner')
            ->add('button_rounded_display', function (Store $row) {
                if ($row->button_rounded) {
            ...
            ->add('dark_mode_display', function (Store $row) {
                if ($row->dark_mode) {
                    return '<span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300">Active</span>';
                }
                return '<span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300">Inactive</span>';
            ...
            ->add('action', function (Store $row) {
                ...
                if (auth()->user() && auth()->user()->hasPermission('view-stores')) {
                    $viewUrl = route('admin.stores.show', $row->id);
                ...
                if (auth()->user() && auth()->user()->hasPermission('delete-stores')) {
                    $deleteUrl = route('admin.stores.destroy', $row->id);
    ...
    public function columns(): array
    {
        return [
            ...
            // Column::make('Banner', 'banner')->sortable()->searchable(),
            // Column::make('Button Rounded', 'button_rounded_display'),
            // Column::make('Dark Mode', 'dark_mode_display'),
    ...
    {
        return [
            Filter::boolean('button_rounded_display', 'button_rounded'),
            Filter::boolean('dark_mode_display', 'dark_mode'),
        ];
    }
    ...
    public function bulkDeleteConfirmed($ids, $model): void
    {
        ...
        try {
            ...
            ActivityLogService::logBulkDelete(Store::class, count($ids), $ids);
