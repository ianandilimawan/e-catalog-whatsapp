...
@section('content')
    <div id="loginContainer"
        ...
        <div
            ...
            <div
                class="hidden lg:flex lg:w-1/2 flex-col justify-between p-12 bg-gradient-to-br from-blue-600/90 to-purple-700/90 dark:from-blue-900/80 dark:to-purple-900/80 text-white relative overflow-hidden">
                <div
                    class="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI4IiBoZWlnaHQ9IjgiPgo8cmVjdCB3aWR0aD0iOCIgaGVpZ2h0PSI4IiBmaWxsPSIjZmZmIiBmaWxsLW9wYWNpdHk9IjAuMDUiLz4KPC9zdmc+')] opacity-20">
                </div>
                <div class="relative z-10">
                    ...
                    @else
                        <div
                            class="h-12 w-12 bg-white/20 backdrop-blur-md rounded-xl shadow-inner border border-white/30 flex items-center justify-center mb-8">
            ...
            <div class="w-full lg:w-1/2 p-8 sm:p-12 lg:p-16 flex flex-col justify-center">
                ...
                <div class="lg:hidden text-center mb-8">
                    ...
                    @else
                        <div
                            class="mx-auto h-12 w-12 bg-gradient-to-br from-blue-600 to-indigo-600 dark:from-blue-500 dark:to-indigo-500 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 flex items-center justify-center">
                            <svg class="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                            </svg>
                ...
                <div class="text-center lg:text-left mb-8">
                    ...
                    <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Please enter the 6-digit OTP code sent to your
                        email.</p>
                </div>

                <!-- OTP Form -->
                <form class="space-y-6" action="{{ route('login.otp.post') }}" method="POST" id="otpForm">
                    @csrf

                    <!-- OTP Field using floating style -->
                    <div>
                        <div class="relative">
                            <input type="text" name="otp" id="otp" required maxlength="6" autocomplete="off"
                                placeholder=" "
                                class="block px-4 pb-3 pt-3 w-full text-center text-3xl font-mono tracking-widest text-zinc-900 bg-transparent rounded-xl border-2 border-zinc-200 appearance-none dark:text-white dark:border-zinc-700 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-500 peer transition-colors"
                                value="{{ old('otp') }}">
                            <label for="otp"
                                class="absolute text-sm text-zinc-500 dark:text-zinc-400 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white/70 dark:bg-zinc-900/70 backdrop-blur px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 start-2 cursor-text rounded-md">Verification
                                Code</label>
                        </div>
                        @error('otp')
                ...
                <form action="{{ route('login.otp.resend') }}" method="POST" class="text-center mt-6" id="resendForm">
                    @csrf
                    <button type="submit" id="resendBtn" disabled
                        class="text-sm font-medium text-zinc-400 dark:text-zinc-500 transition-colors cursor-not-allowed">
                ...
                <div class="text-center mt-10">
                    <p class="text-xs font-medium text-zinc-500 dark:text-zinc-400">
                        © {{ date('Y') }} {{ isset($settings) ? $settings->app_name : 'Admin Panel' }}. All rights
                        reserved.
                    </p>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function startCountdown(duration) {
                ...
                resendBtn.classList.remove('text-blue-600', 'dark:text-blue-400', 'hover:text-blue-800',
                ...
                const interval = setInterval(function() {
                    ...
                    if (--timer < 0) {
                        ...
                        resendBtn.classList.add('text-blue-600', 'dark:text-blue-400',
                            'hover:text-blue-800', 'dark:hover:text-blue-300');
                        resendBtn.classList.remove('text-zinc-400', 'dark:text-zinc-500',
                            'cursor-not-allowed');
                    }
                }, 1000);
            }
