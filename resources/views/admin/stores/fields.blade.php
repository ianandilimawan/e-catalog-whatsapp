...
<div class="mb-8 p-5 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-800/50 relative">
    ...
    <div class="relative z-10">
        <x-select name="user_id" label="Assign Store to User" value="{{ $store->user_id ?? '' }}" :options="$users" />
    </div>
</div>
@endif
...
<div class="grid grid-cols-1 gap-8 mb-4">
    ...
    <div class="bg-white dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 lg:p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <x-input-floating type="text" name="wa_number" label="WhatsApp Number (e.g. 628...)" value="{{ $store->wa_number ?? '' }}" />
        </div>
        <x-textarea-floating name="welcome_message" label="Welcome Message (Displayed on Catalog)" value="{!! $store->welcome_message ?? '' !!}" />
    </div>
    ...
    <div class="bg-white dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 lg:p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-2"><i class="fa fa-info-circle mr-1"></i> Rekomendasi: Gunakan gambar persegi (rasio 1:1) misal 500x500px.</p>
            </div>
            <div>
                <x-filepond name="banner" label="Store Banner" :defaultFile="isset($fileUrls['banner']) ? $fileUrls['banner'] : null" />
                <p class="text-xs text-gray-500 mt-2"><i class="fa fa-info-circle mr-1"></i> Rekomendasi: Gunakan gambar persegi panjang (rasio 3:1) misal 1200x400px.</p>
            </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-100 dark:border-gray-800">
            <div class="col-span-1">
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Theme Color</label>
                <div class="flex items-center gap-3">
                    <input type="color" name="theme_color" value="{{ (isset($store->theme_color) && str_starts_with($store->theme_color, '#')) ? $store->theme_color : '#10b981' }}" class="h-10 w-full max-w-[80px] rounded-lg cursor-pointer border-0 p-0 bg-transparent shadow-sm ring-1 ring-inset ring-gray-200 dark:ring-gray-700">
                </div>
                <p class="text-xs text-gray-500 mt-2">Pick your main brand color.</p>
            </div>
            <div class="col-span-1 flex flex-col justify-center border-t md:border-t-0 md:border-l border-gray-200 dark:border-gray-700 md:pl-6 pt-4 md:pt-0">
