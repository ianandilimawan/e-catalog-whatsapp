...
<head>
    ...
    <title>{{ $store->seo_title ? e($store->seo_title) : ($store->name . ' - ' . config('app.name', 'Katalogin')) }}</title>
    <meta name="description" content="{{ $store->seo_description ? e($store->seo_description) : strip_tags($store->welcome_message) }}">
    <meta property="og:title" content="{{ $store->seo_title ? e($store->seo_title) : ($store->name . ' - ' . config('app.name', 'Katalogin')) }}">
    <meta property="og:description" content="{{ $store->seo_description ? e($store->seo_description) : strip_tags($store->welcome_message) }}">
    @if ($store->logo)
    ...
    @if($store->meta_pixel_id)
        ...
        <noscript><img height="1" width="1" style="display:none" src="https://www.facebook.com/tr?id={{ e($store->meta_pixel_id) }}&ev=PageView&noscript=1"/></noscript>
    @endif
    ...
    @php
        ...
        $logoUrl = $store->logo ? (str_starts_with($store->logo, 'http') ? $store->logo : Storage::url($store->logo)) : null;
        $bannerUrl = $store->banner ? (str_starts_with($store->banner, 'http') ? $store->banner : Storage::url($store->banner)) : null;
    @endphp
    <style>
        :root {
...
<body class="antialiased min-h-screen flex flex-col justify-between relative selection:bg-emerald-500 selection:text-white"
    x-data="catalogApp()"
    :class="{ 'overflow-hidden': isProductModalOpen || isCheckoutModalOpen || isSideMenuOpen }">

    <!-- 1. STICKY TOP NAVIGATION HEADER -->
    <header class="sticky top-0 z-40 w-full backdrop-blur-xl bg-white/90 dark:bg-gray-900/90 border-b border-gray-100 dark:border-gray-800 transition-colors shadow-sm">
        <div class="max-w-5xl mx-auto px-4 pt-2 sm:pt-0 h-16 sm:h-16 flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2 truncate">
                @if($logoUrl)
                    <div class="w-8 h-8 rounded-full overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm flex-shrink-0 bg-white">
                        <img src="{{ $logoUrl }}" alt="Logo" class="w-full h-full object-cover">
                    </div>
                @endif
                <span class="font-bold text-base sm:text-lg truncate max-w-[180px] sm:max-w-xs text-gray-900 dark:text-white">
                    {{ $store->name }}
                </span>
            </div>
    ...
    <div x-show="isSideMenuOpen" class="fixed inset-0 z-[200]" style="display: none;">
        <!-- Backdrop -->
        <div @click="isSideMenuOpen = false"
        ...
        <div x-show="isSideMenuOpen"
            ...
            <div>
                ...
                <div class="p-5 border-b dark:border-gray-800 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-11 h-11 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-800 border dark:border-gray-700 flex items-center justify-center font-bold text-primary text-xl flex-shrink-0">
                            @if($logoUrl)
                        ...
                        <div class="min-w-0">
                            <h3 class="font-bold text-sm text-gray-900 dark:text-white truncate">{{ $store->name }}</h3>
                            {{-- <p class="text-xs text-gray-500 dark:text-gray-400">Katalog Digital Resmi</p> --}}
                        </div>
                    </div>
                    <button @click="isSideMenuOpen = false" class="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800">
                ...
                <nav class="p-4 space-y-1.5">
                    <button @click="isSideMenuOpen = false; window.scrollTo({top:0, behavior:'smooth'})"
                        class="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-left font-semibold text-sm">
                        <span>🏠</span>
                    ...
                    <div x-data="{ catOpen: true }">
                        <button @click="catOpen = !catOpen"
                            class="w-full flex items-center justify-between px-4 py-3 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-left font-semibold text-sm">
                            <div class="flex items-center gap-3">
                                <span>📂</span>
                                <span>Kategori Produk</span>
                            </div>
                            <svg class="w-4 h-4 transition-transform duration-200" :class="catOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div x-show="catOpen" x-collapse class="pl-11 space-y-1 mt-1">
                            <button @click="selectedCategory = 'all'; isSideMenuOpen = false"
                                :class="selectedCategory === 'all' ? 'text-primary font-bold bg-emerald-50 dark:bg-emerald-900/30' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'"
                            ...
                            @foreach($categories as $cat)
                                <button @click="selectedCategory = {{ $cat->id }}; isSideMenuOpen = false"
                                    :class="selectedCategory == {{ $cat->id }} ? 'text-primary font-bold bg-emerald-50 dark:bg-emerald-900/30' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'"
                                    class="w-full text-left py-2 px-3 rounded-lg text-xs transition-colors">
                                    {{ $cat->name }}
                                </button>
                            @endforeach
                    ...
                    <a href="https://wa.me/{{ $cleanWa }}?text={{ urlencode('Halo Admin ' . $store->name . ', saya ingin bertanya...') }}"
                        target="_blank"
                        class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors font-semibold text-sm">
                        <span>💬</span>
                    ...
                    <div class="flex items-center justify-between px-4 py-3">
                        <div class="flex items-center gap-3">
                            <span x-text="localDark ? '🌙' : '☀️'"></span>
                            <span class="text-sm font-semibold">Mode Gelap</span>
                        </div>
                        <button @click="toggleLocalDark()"
                            class="w-11 h-6 rounded-full p-0.5 transition-colors relative"
                            :class="localDark ? 'bg-emerald-600' : 'bg-gray-300'">
                            <div class="w-5 h-5 rounded-full bg-white shadow transition-transform"
                                :class="localDark ? 'translate-x-5' : 'translate-x-0'"></div>
                        </button>
                    </div>
                </nav>
            </div>
            ...
            <div class="p-4 border-t dark:border-gray-800 text-center">
                <a href="{{ url('/') }}" target="_blank" class="text-[11px] text-gray-400 hover:text-primary transition-colors">
    ...
    <section class="w-full max-w-5xl mx-auto px-4 pt-3 sm:pt-4 text-center">
        ...
        @if ($store->welcome_message)
            <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-300 mt-1 max-w-md mx-auto line-clamp-2 leading-relaxed">
    ...
    <main class="max-w-5xl w-full mx-auto px-3 sm:px-4 py-3 flex-1 flex flex-col"
        ...
        <div class="flex overflow-x-auto gap-2 py-3 no-scrollbar mb-2" x-show="allProducts.length > 0">
            <button @click="selectedCategory = 'all'"
                :class="selectedCategory === 'all' ? 'bg-primary text-white shadow-md' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'"
                class="px-4 py-2 rounded-full whitespace-nowrap text-xs sm:text-sm font-bold transition-all btn-custom">
                Semua
            </button>
            @foreach ($categories as $category)
                <button @click="selectedCategory = {{ $category->id }}"
                    :class="selectedCategory == {{ $category->id }} ? 'bg-primary text-white shadow-md' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'"
                    class="px-4 py-2 rounded-full whitespace-nowrap text-xs sm:text-sm font-bold transition-all btn-custom">
                    {{ $category->name }}
                </button>
            @endforeach
        ...
        <div x-show="filteredProducts.length === 0" style="display: none;"
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 max-w-xs">Tidak ada produk yang cocok dengan pencarian atau kategori yang dipilih.</p>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mt-2" x-show="filteredProducts.length > 0">
            <template x-for="product in filteredProducts" :key="product.id">
                <div class="card-custom border rounded-2xl overflow-hidden flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow h-full">
                    <div>
                        ...
                        <div class="relative w-full pt-[100%] bg-gray-100 dark:bg-gray-800 cursor-pointer overflow-hidden rounded-t-2xl"
                            ...
                            <template x-if="product.views_count > 100">
                                <span class="absolute top-2 left-2 bg-red-500 text-white text-[9px] font-bold px-2 py-0.5 rounded-full z-10 shadow">
                                    🔥 TERLARIS
                                </span>
                            </template>
                            ...
                            <template x-if="!product.images || product.images.length === 0">
                                <div class="absolute inset-0 flex items-center justify-center text-gray-400 text-xs font-semibold">No Image</div>
                            </template>
                        </div>
                    ...
                    <div class="p-3 pt-0 mt-auto">
                        <div x-show="!getCartItem(product.id)">
                            <button @click.stop="addToCart(product.id, product.name, product.price)"
                                ...
                                <span>{{ $store->cta_button_text ? e($store->cta_button_text) : 'BELI NOW' }}</span>
                            </button>
                        </div>
                        ...
                        <div x-show="getCartItem(product.id)"
                            ...
                            <span class="text-xs font-extrabold text-gray-900 dark:text-white" x-text="getCartItem(product.id)?.qty"></span>
                            <button @click.stop="updateQuantity(product.id, 1)"
                                class="w-7 h-7 flex items-center justify-center font-bold text-primary bg-white dark:bg-gray-700 btn-custom shadow-sm text-sm">+</button>
                        </div>
                    </div>
    ...
    <!-- 6. CART FLOATING BAR -->
    <div x-show="totalItems > 0" x-transition x-cloak
        class="fixed bottom-0 inset-x-0 z-50 p-3 sm:p-4">
        <div class="max-w-3xl mx-auto">
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 p-3.5 sm:p-4 flex justify-between items-center backdrop-blur-xl">
                <div>
                    <p class="text-[11px] text-gray-500 dark:text-gray-400 font-medium">Total (<span x-text="totalItems"></span> item)</p>
                    <p class="font-black text-base sm:text-lg text-primary">Rp <span x-text="formatRupiah(totalPrice)"></span></p>
                </div>
    ...
    <div x-show="isCheckoutModalOpen" x-cloak
        ...
        <div x-show="isCheckoutModalOpen" @click.outside="isCheckoutModalOpen = false"
            ...
            <div class="flex justify-between items-center mb-4">
                ...
                <button @click="isCheckoutModalOpen = false" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 p-1">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-bold mb-1 text-gray-700 dark:text-gray-300">Nama Lengkap <span class="text-red-500">*</span></label>
                    <input type="text" x-model="customer.name" class="w-full border dark:border-gray-700 rounded-xl p-3 bg-gray-50 dark:bg-gray-800 text-sm focus:ring-2 focus:ring-emerald-500 font-medium"
                        placeholder="Masukkan Nama Anda">
                </div>
                <div>
                    <label class="block text-xs font-bold mb-1 text-gray-700 dark:text-gray-300">Catatan Pesanan <span class="text-xs text-gray-400 font-normal">(Opsional)</span></label>
                    <textarea x-model="customer.notes" class="w-full border dark:border-gray-700 rounded-xl p-3 bg-gray-50 dark:bg-gray-800 text-sm focus:ring-2 focus:ring-emerald-500 font-normal" rows="3"
                        placeholder="Alamat pengiriman, varian khusus, dll..."></textarea>
                </div>
        ...
        <div x-show="isProductModalOpen" @click.outside="closeProductModal()"
            ...
            <div class="relative w-full pt-[80%] sm:pt-[75%] bg-gray-100 dark:bg-gray-800 flex-shrink-0">
                ...
                <template x-if="!activeProduct?.images || activeProduct.images.length === 0">
                    <div class="absolute inset-0 flex items-center justify-center text-gray-400 font-semibold text-sm">No Image</div>
                </template>
            </div>

            <template x-if="activeProduct?.images?.length > 1">
                <div class="flex gap-3 p-3 overflow-x-auto no-scrollbar bg-gray-50 dark:bg-gray-800/60 border-b dark:border-gray-800 flex-shrink-0">
                    <template x-for="(img, index) in activeProduct.images" :key="index">
                        <img :src="img" @click="activeProduct.activeImageIndex = index"
                            :class="activeProduct.activeImageIndex === index ? 'border-primary ring-2 ring-emerald-500' : 'border-transparent opacity-60 hover:opacity-100'"
            ...
            <div class="p-5 overflow-y-auto flex-1 flex flex-col justify-between">
                <div>
                    <h2 class="text-xl font-bold mb-1 text-gray-900 dark:text-white" x-text="activeProduct?.name"></h2>
                    <p class="font-black text-lg text-primary mb-4">Rp <span x-text="activeProduct ? formatRupiah(activeProduct.price) : '0'"></span></p>
            ...
            Alpine.data('catalogApp', () => ({
                ...
                processWhatsApp(waNumber) {
                    ...
                    Object.values(this.cart).forEach(item => {
                        text += `- ${item.name} (${item.qty}x) = Rp ${this.formatRupiah(item.price * item.qty)}\n`;
                    });
    ...
    <!-- 11. FOOTER BRANDING -->
    <footer class="w-full mt-12 py-6 border-t text-center transition-colors {{ $store->dark_mode ? 'border-gray-800 text-gray-500' : 'border-gray-100 text-gray-400' }}">
        <p class="text-xs">
